...
├── src/
│   ├── App.jsx             ← 웹 프로토타입 본체 (모드선택→매장선택→카테고리→아이템커스터마이징→장바구니→결제 흐름)
...
- ✅ 기획: PRD, 콘텐츠 확장 PRD
- ✅ 설계: IA, 데이터 스키마(브랜드/기계형태/장바구니 구조 포함), 와이어프레임
...
- ✅ 확장: 서브웨이 6종 샌드위치 + 소스 단계, 버거킹 6종 버거 + 세트 음료 포함 로직
- 🔜 다음: `docs/키오스크도우미_QA테스트계획.md`의 수동 체크리스트 진행 → 배포 준비 (실기기 Expo Go 테스트, 실사용자 테스트)
